...
The above example demonstrates running a python script locally and having that script transmit itself to the paperspace jobs cluster for further execution. To do this a copy of the local script is modified before transmission to the jobs cluster, in order to strip out the `import gradient` statements and other `gradient` library references. There are also some limitations on the types of import statements that are supported, and the dependencies that are supported in each environment (local vs. remote):

1. You need to use a bare import statement, `import gradient`, and not use the `import gradient as ...` form.
2. The import form `from gradient import ...` is currently not supported.
...
You can run an python script on gradient from the command line as follows:
...
Alternatively you can use the `gradient.run()` fuction in code with a script file name as the first argument:

    import gradient

    gradient.run('myscript.py') # runs myscript on gradient

In code you can provide additional gradient jobs create options in a dict in the second argument to run():
...
The `-m <module path>` option runs the specified library module as a script. This is equivalent to the `-m` option of the `python` executable. Further gradient run option processing is disabled after the `-m` option.

The `-c "python_statement;..."` option runs the specified python statements. This is equivalent to the `-c` option of the `python` executable. Further gradient run option processing is disabled after the `-c` option.
...
See the Paperspace API [jobs create](https://paperspace.github.io/paperspace-node/jobs.html#.create) documentation for more details on the `--workspace` option and related options.
...
- `--project "<project name>"` (defaults to 'gradient')
...
- `--projectId "<existing gradient project id>"`
...
When running python scripts on gradient you may need to provide additional dependencies to your scripts or specify the python version.
...
The `--python 2|3` option allows you specify whether to use `python2` or `python3` when running the script on gradient.
...
are used on the gradient machine to initialize the python environment using the `pipenv` tool, by running `pipenv install` within the container. Note: `pipenv` must already be installed in the container for this option to work. The default container used by gradient already has the `pipenv` package installed.
...
The `--dryrun` option allows you to see the resultant script that will be run on the gradient job runner without actually running it.
...
As mentioned above, you can use the `--dryrun` option to see the resultant commands that will be run on the gradient jobs cluster node for a given set of options, without actually running the commands.

# Default Container

If no `container` option is specified when using `gradient run <script.py>` or the `gradient.run()` function the default container image used is `paperspace/tensorflow-python` on Docker Hub. This container has the tensorflow-gpu libraries installed for both python2 and python3, as well as several other popular packages, including numpy, scipy, scikit-learn, pandas, Pillow and matplotlib.
...
1. Specify your apiKey explicitly on the gradient.run() function or any of the gradient.jobs methods, e.g.:
...
2. Set the package gradient.config option in your python code:

   `gradient.config.gradient_API_KEY = '1qks1hKsU7e1k...'`
