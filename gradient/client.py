...
class API(object):
    ...
    def get_path(self, url):
        api_url = self.api_url if not self.api_url.endswith(
            "/") else self.api_url[:-1]
    ...
    def post(self, url, json=None, params=None, files=None, data=None):
        ...
        response = requests.post(
            path, json=json, params=params, headers=self.headers, files=files, data=data)
    ...
    def put(self, url, json=None, params=None):
        ...
        response = requests.put(
            path, json=json, params=params, headers=self.headers)
        logger.debug("Response status code: {}".format(response.status_code))
        logger.debug("Response content: {}".format(response.content))
        return response
    ...
    def delete(self, url, json=None, params=None):
        ...
        response = requests.delete(
            path, params=params, headers=self.headers, json=json)
